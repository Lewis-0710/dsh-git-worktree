#!/usr/bin/env bash
#==============================================================================
# sync.sh — dsh-git-worktree 上游同步脚本
#==============================================================================
#
# 功能：
...
# 同步策略：
#   ── 策略 1：Patch Apply（优先）──────────────────────────────
#     1. 备份 sync.patch 与 sync.sh 到安全临时目录
#     2. 重置 main 分支到 upstream/main
#     3. 从 main 检出并测试应用 patch
...
for arg in "$@"; do
    case "$arg" in
        ...
        -h|--help)
            ...
            echo "  --skip-test    跳过 pnpm test 验证"
            echo "  -h, --help     显示帮助信息"
            exit 0
            ;;
    esac
...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/LaoYueHanNi/dsh-git-worktree.git"
    exit 1
fi
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_git_worktree_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
